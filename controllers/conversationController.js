...
/**
 * Search for a ONE-TO-ONE conversation with a target user
 * Only allows if the requester is one of the two users
 * GET /api/conversations/one-to-one/search?targetUser=xxx
 */
exports.searchOneToOneConversation = async (req, res) => {
  try {
    ...
    const sortedIds = [requesterId, targetUser].sort()
    const participantPairKey = `${sortedIds[0]}#${sortedIds[1]}`
    ...
    return res.status(200).json({ conversation: conversations[0] })
  } catch (error) {
